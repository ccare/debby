...
function S3(knoxClient) {

	// s3Client provides a wrap around knox to facilitate uploads
	var s3Client = s3Lib.fromKnox(knoxClient);

	this.deleteMultiple = function(objects, callback) {
		knoxClient.deleteMultiple(objects, function(err, res) {
    		if (err) {
    			callback(err);
    		} else {
    			callback();
	...
	this.put = function(object, content, callback) {
		var req = knoxClient.put(object, {
...
function Indexer(knoxClient) {
	...
	function listDebsAndMetas(callback) {
		async.parallel({
		    debArchives: listObjects('deb/', '.deb', 'deb/'),
		    metaFiles: listObjects('meta/', '.meta', 'meta/'),
		    repoFiles: listObjects('repo/', '', 'repo/')
		}, callback);
	}
	...
	function rebuildReleaseFile(callback) {
		...
		buildRelease(function(contents) {
			s3.put(releaseObject, contents, callback)
		})
	}
...
function listObjects(prefix, extension, rootPath) {
