...
function Indexer(knoxClient) {
	...
	function listDebsAndMetas(callback) {
		async.parallel({
		    debArchives: listObjects(BUCKET, 'deb/', '.deb'),
		    metaFiles: listObjects(BUCKET, 'meta/', '.meta'),
		    repoFiles: listObjects(BUCKET, 'repo/', '')
		}, callback);
	}
...
function listObjects2(bucket, prefix, extension) {
	return function(callback) {
